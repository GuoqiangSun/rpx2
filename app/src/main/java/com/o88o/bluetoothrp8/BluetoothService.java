...
public class BluetoothService extends Service {
    ...
    public void connect(final ScanResult scanResult) {
        bleManager.connect(scanResult, new BleGattCallback() {
            ...
            @Override
            public void onDisConnected2Reconnect(BluetoothGatt gatt, int status, BleException exception){
                runOnMainThread(new Runnable() {
                    @Override
                    public void run() {
                        if (mCallback != null) {
                            mCallback.onDisConnected2Reconnect();
                        }
                    }
                });
            }
    ...
    public boolean isServiceDiscover(){
        return bleManager.isServiceDiscovered();
    }
